...
public interface VisionTestRunRepository extends CrudRepository<VisionTestRun, Long> {
	...
	@Query("SELECT count(distinct subjectId) FROM VisionTestRun tr WHERE subjectAge = :age")
    public int findRunCountByAge(@Param("age") int age);
	
	@Query("SELECT count(distinct subjectId) FROM VisionTestRun tr WHERE subjectAge = :age and score < :score")
